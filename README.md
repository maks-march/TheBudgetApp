...
1. Проект использует Gradle Wrapper (8.9) — файлы `gradlew`, `gradlew.bat`,
   `gradle/wrapper/*` обязательны, версия закреплена и совместима с AGP 8.5.2
   (системный Gradle 9.x НЕ подходит).
2. `local.properties` в git/архив не входит (машино-зависимый): Android Studio
   создаёт его сам при первом открытии/синхронизации проекта. Вручную:
   `sdk.dir=C\:\\Users\\<имя>\\AppData\\Local\\Android\\Sdk` (двойные бэкслеши),
   либо переменная окружения `ANDROID_HOME`.
3. Сборка из терминала: `gradlew.bat assembleDebug` (Windows) /
